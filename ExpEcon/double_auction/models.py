...
class Group(BaseGroup):
    ...
    def allotment(self): # タイプに応じて初期保有を割り当てる
    ...
    def func_chosen_utility(self): # 何ラウンド目の利得を用いるかを決める
        for p in self.get_players():
...
class Player(BasePlayer):
    ...
    chosen_utility = models.FloatField() # 最終的につかう利得の保存場所
    chosen_round = models.IntegerField(initial=np.random.randint(1, 5)) #どのラウンドの利得を使ったか。サーバーを立ち上げた時に決まる
    ...
    history_partner = models.LongStringField(initial='0,') # 取引相手
    ...
    def allot_role(self):
        ...
        if self.id_in_group <= 10: #グループIDが10以下の場合はタイプA
            self.roll = '1-A-F'
        elif self.id_in_group > 10: #グループIDが10より大きい場合はタイプB
            self.roll = '2-G-L'
